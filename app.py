...
def main():
    ...
    if "📡 Live Monitor" in tab_names:
        with tabs[tab_names.index("📡 Live Monitor")]:
            ...
            refresh_rate = st.slider("Refresh Interval (seconds)", 1, 5, 1)
            ...
            def one_cycle_read_and_plot():
                ...
                df = pd.DataFrame(data_manager.get_patient_vitals_history(patient_id, limit=200))
                if df.empty:
                ...
                fig = make_subplots(
                    ...
                    subplot_titles=("ECG", "SpO₂", "Systolic BP", "Diastolic BP")
                )

                # Sensor colors
                sensor_colors = {"ECG": "lime", "SpO2": "cyan", "BP_SYS": "orange", "BP_DIA": "red"}
                ...
                sensor_to_row = {"ECG": 1, "SpO2": 2, "BP_SYS": 3, "BP_DIA": 4}
                ...
                for sensor in ["ECG", "SpO2", "BP_SYS", "BP_DIA"]:
                    ...
                    fig.add_trace(
                        ...
                        row=sensor_to_row[sensor], col=1
                    )
                ...
                # Sliding window (last 50 samples visible)
                window_size = 50
                x_end = df["timestamp"].iloc[-1]
                x_start = df["timestamp"].iloc[-window_size] if len(df) > window_size else df["timestamp"].iloc[0]

                fig.update_layout(
                    title="📈 ICU Multi-Sensor Real-Time Monitor",
                    plot_bgcolor="black",
                    paper_bgcolor="black",
                ...
                fig.update_xaxes(range=[x_start, x_end], showgrid=True, gridcolor="gray", tickformat="%H:%M:%S")

                # Apply fixed y-axis per subplot
                fig.update_yaxes(range=y_ranges["ECG"], row=1, col=1, title="bpm")
                fig.update_yaxes(range=y_ranges["SpO2"], row=2, col=1, title="%")
                fig.update_yaxes(range=y_ranges["BP_SYS"], row=3, col=1, title="mmHg")
                fig.update_yaxes(range=y_ranges["BP_DIA"], row=4, col=1, title="mmHg")
            ...
            # Continuous refresh at selected interval
            st_autorefresh(interval=refresh_rate * 1000, key="ecg_refresh")
            one_cycle_read_and_plot()

            # Update digital twin & alerts (non-blocking summary sidebar)
        ...
        with tabs[tab_names.index("🔮 Forecasting Studio")]:
            ...
            with colB:
                uploader = st.file_uploader(
                    "(Optional) Upload CSV with columns: timestamp, systolic_bp, diastolic_bp, body_temperature",
                    type=["csv"], key="forecast_csv"
                )
